...
fun ModificarFormulario(appVM: AppVM, formVM: FormVM) {
        ...
        Button(onClick = {
            routineScope.launch(Dispatchers.IO) {
                ...
                dao.updatePlace(
                    ...
                    lugar,
                    orden.toInt(),
                    BigDecimal(precioAlojamiento.toDouble() / formVM.dolar.value).setScale(
                        2,
                        RoundingMode.HALF_EVEN
                    ).toDouble(),
                    BigDecimal(precioMovilizacion.toDouble() / formVM.dolar.value).setScale(
                        2,
                        RoundingMode.HALF_EVEN
                    ).toDouble(),
                    comentarios
